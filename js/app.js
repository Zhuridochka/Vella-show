(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".hero__slider--about")) new swiper_core_Swiper(".hero__slider--about", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        const contentSliderPreviews = new swiper_core_Swiper(".content-slider__previews", {
            modules: [ Thumb ],
            observer: true,
            observeParents: true,
            slidesPerView: 6,
        ...
        if (document.querySelector(".content-slider__main")) {
            new swiper_core_Swiper(".content-slider__main", {
                modules: [ Navigation, Thumb ],
                thumbs: {
                    swiper: contentSliderPreviews
                },
                navigation: {
                    prevEl: ".main-content-slider__button-prev",
                    nextEl: ".main-content-slider__button-next"
                },
                on: {}
            });
        }
        if (document.querySelector(".comment-reviews__slider")) new swiper_core_Swiper(".comment-reviews__slider", {
            modules: [ Navigation, Pagination ],
            slidesPerView: 5,
            spaceBetween: 8,
            speed: 600,
            pagination: {
                el: ".navigation-roof-reviews__pagination",
                type: "custom",
                renderCustom: function(swiper, current, total) {
                    return `Viewed ${current} / ${total}`;
                },
                clickable: true
            },
            navigation: {
                prevEl: ".navigation-roof-reviews__button-prev",
                nextEl: ".navigation-roof-reviews__button-next"
            },
            breakpoints: {
                319: {
                    slidesPerView: 4,
        ...
        if (document.querySelector(".equipment__slider")) new swiper_core_Swiper(".equipment__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: "auto",
    ...
    function windowLoaded() {
        let documentActions = e => {
            ...
            if (targetElement.closest(".filter-button__body")) {
                if (window.innerWidth < 641) document.documentElement.classList.toggle("filter-open");
            } else if (!targetElement.closest(".filter__body")) document.documentElement.classList.remove("filter-open");
            if (targetElement.closest(".sorting-button__body")) {
                if (window.innerWidth < 641) document.documentElement.classList.toggle("sorting-open");
            } else if (!targetElement.closest(".sorting")) document.documentElement.classList.remove("sorting-open");
        };
        ...
        function addClassActive() {
            ...
            footerLinks.forEach((elem => {
                if (elem.href === currentUrl) elem.classList.add("active");
            }));
        }
        addClassActive();
        if (window.innerWidth < 479) {
            const articles = document.querySelectorAll(".cooperation__item.item-guarantees");
            if (articles.length > 0) articles.forEach(((article, index) => {
                console.log(`Обробка статті #${index + 1}`);
                const icon = article.querySelector(".item-guarantees__icon");
                const top = article.querySelector(".item-guarantees__top");
                if (icon && top) {
                    console.log("Переміщення іконки...");
                    top.appendChild(icon);
                } else console.error(`Елементи .item-guarantees__icon або .item-guarantees__top не знайдено у статті #${index + 1}`);
            })); else console.log("Жодного елемента <article> не знайдено.");
        }
        function updateDataAttributeBaseOnHeight() {
            const itemCatalog = document.querySelector(".item-catalog");
            if (!itemCatalog) return;
            const contentItems = document.querySelector(".content-goods__items");
            const itemCatalogHeight = itemCatalog.offsetHeight;
            if (contentItems) {
                const calculateHeight = itemCatalogHeight * 5;
                contentItems.dataset.showmoreContent = calculateHeight;
            }
        }
        updateDataAttributeBaseOnHeight();
        window.addEventListener("resize", updateDataAttributeBaseOnHeight);
